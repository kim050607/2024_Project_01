using System;                                    //Action 을 사용하기 위해서 
...
public class GamerManager : MonoBehaviour
{
    ...
    public static event Action<int> OnBestScoreChanged;      //최고점수가 변경 되었을때 이벤트를 변경시킨다.
    ...
    void Update()
    {
        if(isGen == false)                            //isGen 플래그가 false 일 경우
        {
            ...
            if(timeCheck <= 0.0f)                     //0초 이하가 되었을 경우
            {
                ...
                GameObject Temp = Instantiate(circleObject[0]); //프리팹 생성후 Temp 오브젝트에 넣는다.
